...
def permissions():
    form = SQLFORM.factory(
        Field('action',
              requires=IS_IN_SET((('add',T('Add permissions')), ('remove', T('Remove permissions'))), zero=None), default=True, widget=SQLFORM.widgets.options.widget),
        ...
        Field('permissions', 'list:string',
              requires= IS_IN_SET(('w2a_read', 'w2a_create', 'w2a_select', 'w2a_edit', 'w2a_delete'), multiple=True ), widget=SQLFORM.widgets.checkboxes.widget)
    )
